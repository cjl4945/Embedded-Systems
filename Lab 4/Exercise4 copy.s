...
;Date:  02/08/19
;Class:  CMPE-250
;Section:  <Lab Section: 1, Friday, 	2:18 pm>
;---------------------------------------------------------------
...
;>>>>> begin main program code <<<<<
			BL		InitData		;call initdata
While		BL		LoadData		;call loaddata
			BCS		EndWhile		;if c flag is set branch to EndWhile
			LDR		R1,=P			;load variable p into a register
			LDR		R1,[R1,#0]
			LDR		R0,=Q			;load variable q into a register
			LDR		R0,[R0,#0]
			BL		DIVU			;call the divu subroutine 
			BCC		Els				;if c flag is clear branch to else
			MOVS	R2,#0
			MOVS	R3,#0
			SUBS	R2,R2,#1
			SUBS	R3,R3,#1
...
DIVU		PROC	{R2-R14}		;register 2-14 are unchaged
			PUSH    {R2} 
            MOVS    R2,#0 
            CMP     R0,#0
